var vg_1 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/ghg.vg.json";
vegaEmbed("#areachart", vg_1, {mode: "vega-lite",actions: false}).then(console.log).catch(console.warn)

var vg_2 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/piechart.vg.json";
vegaEmbed("#piechart", vg_2, {mode: "vega-lite"},{"actions": false}).then(console.log).catch(console.warn)

var vg_3 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/bubblechart.vj.json";
vegaEmbed("#bubblechart", vg_3, {mode: "vega-lite"},{"actions": false}).then(console.log).catch(console.warn)

var vg_4 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/worldmap.vg.json";
vegaEmbed("#worldmap", vg_4, {mode: "vega-lite"},{"actions": false}).then(console.log).catch(console.warn)


var vg_5 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/tempbar.vg.json";
vegaEmbed("#tempbar", vg_5, {mode: "vega-lite"},{"actions": false}).then(console.log).catch(console.warn)

var vg_6 = "https://raw.githubusercontent.com/ujay0005/3179/main/json/stack.vg.json";
vegaEmbed("#stack", vg_6, {mode: "vega-lite"},{"actions": false}).then(console.log).catch(console.warn)



